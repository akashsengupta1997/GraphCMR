...
def evaluate_movi_sports(model,
    ...
    for batch_num, samples_batch in enumerate(tqdm(eval_dataloader)):
        ...
        pred_vertices_projected2d = undo_keypoint_normalisation(pred_vertices_projected2d,
                                                                input.shape[-1])
        pred_vertices_smpl_projected2d = orthographic_project_torch(pred_vertices_smpl,
                                                                    pred_camera)
        pred_vertices_smpl_projected2d = undo_keypoint_normalisation(
            pred_vertices_smpl_projected2d, input.shape[-1])
        ...
        if vis_every_n_batches is not None:
            if batch_num % vis_every_n_batches == 0:
                ...
                plt.subplot(243)
                plt.imshow(vis_imgs[0])
                plt.scatter(pred_vertices_smpl_projected2d[0, :, 0],
                            pred_vertices_smpl_projected2d[0, :, 1], s=0.1, c='r')
...
if __name__ == '__main__':
    ...
    dataset = MoViShapeEvalDataset(dataset_path, img_wh=config.INPUT_RES)
    print("Eval examples found:", len(dataset))

    # Metrics
    metrics = ['pve-t', 'pve-t_scale_corrected']

    save_path = '/data/cvfs/as2562/GraphCMR/evaluations/movi_shape'
